...
Component({
  ...
  methods: {
    ...
    // },
    _beginToScroll: function _beginToScroll() {
      if (!this._lastScrollLeft) {
        this._lastScrollLeft = this._pos && (this._pos.left || 0)
      }
    },
    _clearList: function _clearList(cb) {
      ...
      pos.afterWidth = pos.minLeft = pos.minLeft = 0;
    ...
    _scrollViewDidScroll: function _scrollViewDidScroll(e, force) {
      ...
      //   return;
      // }
      if (!this._pos.width || !this.sizeArray.length) {
        this._clearList(e.detail.cb);
        return;
      }
      ...
      var scrollDistance = Math.abs(scrollLeft - this._lastScrollLeft);
      // if (!force && Math.abs(scrollTop - pos.top) < pos.height * 1.5) {
      //   this._log('【not exceed height');
      //   return;
      // }
      if (!force && Math.abs(scrollLeft - pos.left) < pos.width * 1.5) {
        this._log('【not exceed width');
        return;
      }
      ...
      this._calcViewportIndexes(scrollLeft, scrollTop, function (beginIndex, endIndex, minLeft, afterWidth, maxLeft) {
        ...
        //   return;
        // }
        if (!force && pos.beginIndex === beginIndex && pos.endIndex === endIndex && pos.minLeft === minLeft && pos.afterWidth === afterWidth) {
          that._log('------------is the same beginIndex and endIndex');
          return;
        }
        // 如果这次渲染的范围比上一次的范围小，则忽略
        // that._log('【check】before setData, old pos is', pos.minTop, pos.maxTop, minTop, maxTop);
        that._log('【check】before setData, old pos is', pos.minLeft, pos.maxLeft, minLeft, maxLeft)
    ...
    _calcViewportIndexes: function _calcViewportIndexes(left, top, cb) {
      ...
      // this._getBeforeSlotHeight(function () {
        this._getBeforeSlotWidth(function () {
        var _that$__calcViewportI = that.__calcViewportIndexes(left, top),
            ...
            minLeft = _that$__calcViewportI.minLeft,
            // afterHeight = _that$__calcViewportI.afterHeight,
            afterWidth = _that$__calcViewportI.afterWidth,
            // maxTop = _that$__calcViewportI.maxTop;
            maxLeft = _that$__calcViewportI.maxLeft;

        if (cb) {
          // cb(beginIndex, endIndex, minTop, afterHeight, maxTop);
          cb(beginIndex, endIndex, minLeft, afterWidth, maxLeft);
        }
      });
    },
    ...
    //   }
    // },
    _getBeforeSlotWidth: function _getBeforeSlotWidth(cb) {
      if (typeof this.data.beforeSlotWidth !== 'undefined') {
        if (cb) {
          cb(this.data.beforeSlotWidth);
        }
      } else {
        this.reRender(cb);
      }
    ...
    //   }
    // },
    _getAfterSlotWidth: function _getAfterSlotWidth(cb) {
      if (typeof this.data.afterSlotWidth !== 'undefined') {
        if (cb) {
          cb(this.data.afterSlotWidth);
        }
      } else {
        this.reRender(cb);
      }
    ...
    //   };
    // },
    _getIndexes: function _getIndexes(minLeft, maxLeft) {
      if (minLeft === maxLeft && maxLeft === 0) {
        return {
          beginIndex: -1,
          endIndex: -1
        };
      }
      var startLine = Math.floor(minLeft / RECT_SIZE);
      var endLine = Math.ceil(maxLeft / RECT_SIZE);
      var rectEachLine = Math.floor(this.data.width / RECT_SIZE);
      var beginIndex = void 0;
      var endIndex = void 0;
      var sizeMap = this.sizeMap;
    ...
    __calcViewportIndexes: function __calcViewportIndexes(left, top) {
      ...
      var beforeSlotWidth = this.data.beforeSlotWidth || 0;
      // 和direction无关了
      ...
      var minLeft = left - pos.width * SHOW_SCREENS - beforeSlotWidth;
      var maxLeft = top + pos.width * SHOW_SCREENS - beforeSlotWidth;
      ...
      //   minTop = 0;
      // }
      if (maxLeft > this.totalWidth) {
        minLeft -= maxLeft - this.totalWidth;
        maxLeft = this.totalWidth;
      }
      if (minLeft < beforeSlotWidth) {
        maxLeft += Math.min(beforeSlotWidth - minLeft, this.totalWidth);
        minLeft = 0;
      }
      // 计算落在minTop和maxTop之间的方格有哪些
      // var indexObj = this._getIndexes(minTop, maxTop);
      var indexObj = this._getIndexes(minLeft, maxLeft);
      ...
      var maxLeftFull = this.sizeArray[endIndex].beforeWidth + this.sizeArray[endIndex].width;
      var minLeftFull = this.sizeArray[beginIndex].beforeWidth;

      // console.log('render indexes', beginIndex, endIndex)
      // var afterHeight = this.totalHeight - maxTopFull;
      var afterWidth = this.totalWidth - maxLeftFull;
      return {
        beginIndex: beginIndex,
        endIndex: endIndex,
    ...
    setItemSize: function setItemSize(size) {
      ...
      if (size.totalWidth !== this.totalWidth) {
        this.setData({
          totalWidth: size.totalWidth,
          useInPage: this.useInPage || false
        });
        this.totalWidth = size.totalWidth;
      }
    },
        ...
        this.reRender(function () {
          that._scrollViewDidScroll({
            detail: {
              ...
              scrollLeft: that.currentScrollLeft || that.data.scrollLeft || 0,
    ...
    _initPosition: function _initPosition(cb) {
      ...
      that._pos = {
        ...
        width: Math.max(500, this.data.width),
    ...
    _widthChanged: function _widthChanged(newVal) {
      ...
      this._pos.width = Math.max(500, this.data.width),
      this.forceUpdate();
      return newVal;
    },
    ...
    reRender: function reRender(cb) {
      ...
      var beforeSlotWidth = void 0;
      var afterSlotWidth = void 0;
      var that = this;
      ...
      function newCb() {
        ...
        //   });
        // }
        if (that._lastBeforeSlotWidth !== beforeSlotWidth || that._lastAfterSlotWidth !== afterSlotWidth) {
          that.setData({
            hasBeforeSlotWidth: true,
            hasAfterSlotWidth: true,
            beforeSlotWidth: beforeSlotWidth,
            afterSlotWidth: afterSlotWidth
          });
        }
        // that._lastBeforeSlotHeight = beforeSlotHeight;
        // that._lastAfterSlotHeight = afterSlotHeight;
        that._lastBeforeSlotWidth = beforeSlotWidth;
        that._lastAfterSlotWidth = afterSlotWidth;
    ...
    //   }
    // },
    _setInnerBeforeAndAfterWidth: function _setInnerBeforeAndAfterWidth(obj) {
      if (typeof obj.beforeWidth !== 'undefined') {
        this._tmpBeforeWidth = obj.beforeWidth;
      }
      if (obj.afterWidth) {
        this._tmpAfterWidth = obj.afterWidth;
      }
    },
    _recycleInnerBatchDataChanged: function _recycleInnerBatchDataChanged(cb) {
      ...
      // }
      if (typeof this._tmpBeforeWidth !== 'undefined') {
        var setObj = {
          innerBeforeWidth: this._tmpBeforeWidth || 0,
          innerAfterWidth: this._tmpAfterWidth || 0
        };
        if (typeof this._tmpInnerScrollLeft !== 'undefined') {
          setObj.innerScrollLeft = this._tmpInnerScrollLeft;
        }
        var pageObj = {};
        var hasPageData = false;
        if (typeof this._currentSetDataKey !== 'undefined') {
        ...
        // this._tmpAfterHeight = undefined;
        // this._tmpInnerScrollTop = undefined;
        this._tmpBeforeWidth = undefined;
        this._tmpAfterWidth = undefined;
        this._tmpInnerScrollLeft = undefined;
      }
    },
    ...
    //   });
    // },
    _renderByScrollLeft: function _renderByScrollLeft(scrollLeft) {
      // 先setData把目标位置的数据补齐
      this._scrollViewDidScroll({
        detail: {
          scrollLeft: scrollLeft,
          scrollTop: this._pos.scrollTop,
          ignoreScroll: true
        }
      }, true);
      if (this.data.scrollWithAnimation) {
    ...
    //   // }
    //   if (typeof this._lastScrollLeft === 'undefined') {
    //     this._lastScrollLeft = this.data.scrollLeft;
    //   }
    ...
    //   //   return newVal;
    //   // }
    //   if (Math.abs(newVal - this._lastScrollLeft) < this._pos.height) {
    //     this.setData({
    //       innerScrollLeft: newVal
    //     });
    //     return newVal;
    //   }
    ...
    //   return newVal;
    // },
    _scrollLeftChanged: function _scrollLeftChanged(newVal, oldVal) {
      var _this5 = this;

      // if (newVal === oldVal && newVal === 0) return
      if (!this._isInitScrollLeft && newVal === 0) {
        this._isInitScrollLeft = true;
        return newVal;
      }
      this.currentScrollLeft = newVal;
      if (!this._isReady) {
        if (this._scrollLeftTimerId) {
          clearTimeout(this._scrollLeftTimerId);
        }
        this._scrollLeftTimerId = setTimeout(function () {
          _this5._scrollLeftChanged(newVal, oldVal);
        }, 10);
        return newVal;
      }
      this._isInitScrollLeft = true;
      this._scrollLeftTimerId = null;
      // this._lastScrollLeft = oldVal;
      if (typeof this._lastScrollLeft === 'undefined') {
        this._lastScrollLeft = this.data.scrollLeft;
      }
      // 滑动距离小于一个屏幕的宽度, 直接setData
      if (Math.abs(newVal - this._lastScrollLeft) < this._pos.width) {
        this.setData({
          innerScrollLeft: newVal
        });
        return newVal;
      }
      if (!this._isScrollLeftChanged) {
        // 首次的值需要延后一点执行才能生效
        setTimeout(function () {
          _this5._isScrollLeftChanged = true;
          _this5._renderByScrollLeft(newVal);
        }, 10);
      } else {
        this._renderByScrollLeft(newVal);
      }
      return newVal;
    },
    _scrollToIndexChanged: function _scrollToIndexChanged(newVal, oldVal) {
      ...
      var calScrollLeft = rect.left + (this.data.beforeSlotWidth || 0);
      this.currentScrollLeft = calScrollLeft;
      ...
      //   return newVal;
      // }
      if (Math.abs(calScrollLeft - this._lastScrollLeft) < this._pos.width) {
        this.setData({
          innerScrollLeft: calScrollLeft
        });
        return newVal;
      }
      if (!this._isScrollToIndexChanged) {
        setTimeout(function () {
          ...
          _this6._renderByScrollLeft(calScrollLeft);
        }, 10);
      } else {
        // this._renderByScrollTop(calScrollTop);
        this._renderByScrollLeft(calScrollLeft);
      }
      return newVal;
    },
    ...
    boundingClientRect: function boundingClientRect(idx) {
      ...
      return {
        ...
        left: this.sizeArray[idx].beforeWidth,
        top: 0,
    ...
    getIndexesInViewport: function getIndexesInViewport(inViewportPx) {
      ...
      for (var i = 0; i < this.sizeArray.length; i++) {
        ...
        //   indexes.push(i);
        // }
        if (this.sizeArray[i].beforeWidth + this.sizeArray[i].width >= minTop && this.sizeArray[i].beforeWidth <= maxLeft) {
          indexes.push(i);
        }
        // if (this.sizeArray[i].beforeHeight > maxTop) break;
        if (this.sizeArray[i].beforeWidth > maxLeft) break;
      }
      return indexes;
    },
