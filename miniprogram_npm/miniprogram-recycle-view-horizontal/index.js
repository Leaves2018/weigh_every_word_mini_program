...
RecycleContext.prototype._recalculateSize = function (list) {
  console.log('_recalculateSize() is called')
  // 遍历所有的数据
  // 应该最多就千量级的, 遍历没有问题
  var sizeMap = {};
  ...
  for (var i = 0; i < listLen; i++) {
    ...
    if (offsetTop + itemSize.height > compData.height) {
      column = 0;
      offsetTop = itemSize.top;
      // Fixed issue #22
      if (sizeArray.length >= 2) {
        offsetLeft += sizeArray[sizeArray.length - 2].left || 0; // 加上最后一个数据的宽度
      } else {
        offsetLeft += itemSize.left;
      }
      ...
      if (offsetLeft >= RECT_SIZE * (column + 1)) {
        ...
        line += parseInt((offsetLeft - RECT_SIZE * line) / RECT_SIZE, 10);
      ...
      if (i === 0) {
        itemSize.beforeWidth = 0;
      } else {
        var prevItemSize = sizeArray[sizeArray.length - 2];
        itemSize.beforeWidth = prevItemSize.beforeWidth + prevItemSize.width;
      }
    } else {
      ...
      if (i === 0) {
        ...
        itemSize.beforeWidth = sizeArray[sizeArray.length - 2].beforeWidth;
      }
    }
    var key = line + '.' + column;
    if (!sizeMap[key]) {
    ...
    // fix: 当区块比较大时，会缺失块区域信息
    if (listLen - 1 === i && itemSize.width > RECT_SIZE) {
      var _lastIdx = line;
      offsetLeft += itemSize.width;
      column += parseInt((offsetTop - RECT_SIZE * column) / RECT_SIZE, 10);
      for (var _idx = _lastIdx; _idx <= column; _idx++) {
        var _key2 = line + '.' + _idx;
        if (!sizeMap[_key2]) {
          sizeMap[_key2] = [];
        }
  ...
  var obj = {
    array: sizeArray,
    map: sizeMap,
    totalWidth: sizeArray.length ? sizeArray[sizeArray.length - 1].beforeWidth + sizeArray[sizeArray.length - 1].width : 0
  };
...
RecycleContext.prototype.getBoundingClientRect = function (index) {
  ...
  return {
    ...
    left: sizeArray[index].beforeWidth,
    top: 0,
    width: sizeArray[index].width,
    height: sizeArray[index].height,
  };
...
// };
RecycleContext.prototype.getTotalWidth = function () {
  this.checkComp();
  return this.comp.getTotalWidth();
}
...
module.exports = function (e, cb) {
  ...
  // });
  comp._setInnerBeforeAndAfterWidth({
    beforeWidth: pos.minLeft,
    afterWidth: pos.afterWidth,
  })
