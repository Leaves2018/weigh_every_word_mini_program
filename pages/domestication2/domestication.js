...
Component({
  ...
  methods: {
    onLoad: function() {
      // 页面加载时，从缓存中读取上一次修改记录（如果有的话）
      let lexiconConfiguration = wx.getStorageSync('lexicon_configuration');
      if (typeof(lexiconConfiguration) === 'string') {
        wx.startPullDownRefresh(); // 本地没有，则认为需要初始化
      } else {
        this.setData({
          keys: Object.keys(lexiconConfiguration).sort((a, b) =>
            lexiconConfiguration[a].order - lexiconConfiguration[b].order
          ),
          lexiconConfiguration: lexiconConfiguration,
        })
      }
    },
    onUnload: function() {
      wx.setStorage({
        key: "lexicon_configuration",
        data: this.data.lexiconConfiguration,
      })
      app.familiarTrie.save();
    },
    // 下拉刷新时，从数据库获取最新词典配置可选项
    onPullDownRefresh: function() {
      console.log('onPullDownRefresh() is called')
      var that = this;
      db.collection('common_configuration').doc('familiar_lexicon').get({
        success: function(res) {
          let cloudLexiconConfiguration = res.data.lexicon;
          let localLexiconConfiguration = that.data.lexiconConfiguration;
          // 继承原有的checked属性
          // 用于排序的order将被刷新
          // 忽略了本地比云端多的情况（原则上不会出现）
          let localIds = Object.keys(localLexiconConfiguration);
          if (localIds.length !== 0) {
            localIds.forEach(_id => {
              cloudLexiconConfiguration[_id]['checked'] = localLexiconConfiguration[_id].checked;
            })
          }
          that.setData({
            keys: Object.keys(cloudLexiconConfiguration).sort((a, b) =>
              cloudLexiconConfiguration[a].order - cloudLexiconConfiguration[b].order
            ), // 设置wx:for变量，并按照order排序
            lexiconConfiguration: cloudLexiconConfiguration,
          })
    ...
    check: function(e) {
      ...
      let lexiconConfiguration = this.data.lexiconConfiguration;
      lexiconConfiguration[key].checked = !lexiconConfiguration[key].checked;
      this.dealFile(key, this.data.lexiconConfiguration[key].checked);
      this.setData({
        lexiconConfiguration: lexiconConfiguration,
